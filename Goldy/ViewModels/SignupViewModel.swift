...
class SignupViewModel: ObservableObject {
    ...
    @Published var userType: User.UserType = .customer
    ...
    func signUp() {
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            .tryMap { [weak self] data, response -> Data in
                ...
                case 500:
                    if let errorString = String(data: data, encoding: .utf8) {
                        if errorString.contains("Unique constraint") || errorString.contains("email") {
                            throw NetworkError.conflict("An account with this email already exists")
                        }
                    }
            ...
            .sink { [weak self] completion in
                ...
                print("Signup successful for user: \(loginResponse.user.email)")
