...
enum TermsMode {
    case autoGenerated  // AI-generated terms based on vendor type
    case basicTerms     // Minimal essential terms (cancellation, refunds, disputes)
    case uploadOwn      // User uploads their own contract
}
...
class AddVendorViewModel: ObservableObject {
    ...
    @Published var hasSearched = false // Track if a search has been performed
    ...
    @Published var termsMode: TermsMode = .autoGenerated
    @Published var uploadedContractURL: URL? = nil
    @Published var uploadedContractName: String = ""
    ...
    func generateTerms() {
        generateTermsForMode(termsMode)
    }

    func generateTermsForMode(_ mode: TermsMode) {
        ...
        case .basicTerms:
            // Essential terms only - minimum protection
            termsSections = [
                TermsSection(
                    title: "Payment Terms",
                    content: generatePaymentTerms(formattedTotal: formattedTotal)
                ...
                TermsSection(
                    ...
                    content: generateBasicCancellationPolicy()
                ),
                TermsSection(
                    title: "Dispute Resolution",
                    content: generateBasicDisputeTerms()
                )
            ]

        case .uploadOwn:
            // User will upload their own - clear sections
            termsSections = []
        }
    }

    private func generateBasicCancellationPolicy() -> String {
        return """
        If either party needs to cancel:
        • Customer cancellation: Refund amount depends on notice given and work completed
        • Vendor cancellation: Full refund of funds held in escrow
        • Both parties agree to communicate promptly about any changes
        """
    }

    private func generateBasicDisputeTerms() -> String {
        return """
        If there's a disagreement:
        • Both parties will first try to resolve it directly
        • If unresolved, Go Hard's support team can help mediate
        • All funds remain in escrow until disputes are settled
        """
    }
