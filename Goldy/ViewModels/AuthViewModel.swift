    ...
    func login() {
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            ...
            .sink { [weak self] completion in
                ...
                print("✅ Login successful for user: \(loginResponse.user.email)")
                print("✅ Token stored: \(loginResponse.token)")
                print("✅ UserId stored: \(loginResponse.user.id)")
                print("✅ UserType: \(loginResponse.user.userType)")
                
                self?.token = loginResponse.token
                self?.storedUserId = loginResponse.user.id
