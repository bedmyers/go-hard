    ...
    var body: some View {
        ...
        .onChange(of: viewModel.isSignedUp) { isSignedUp in
            if isSignedUp {
                ...
                print("DEBUG: SignupView calling appState.login")
...
private struct SignupMainContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            UserTypePickerView(userType: $viewModel.userType, isLoading: viewModel.isLoading)
                .padding(.bottom, 16)
...
private struct UserTypePickerView: View {
    @Binding var userType: User.UserType
    let isLoading: Bool
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .padding(.vertical, 12)
                .background(isSelected ? Color.black : Color.white.opacity(0.8))
                .cornerRadius(8)
                .overlay(
                    RoundedRectangle(cornerRadius: 8)
                        .stroke(Color.black.opacity(0.2), lineWidth: isSelected ? 0 : 1)
                )
        }
        .disabled(isLoading)
    }
